...
<template>
    <div v-if="!isSupported" class="text-sm text-muted-foreground">
        Kunci akses belum didukung oleh browser ini.
    </div>
    ...
    >
        <div class="grid gap-2">
            <Label for="passkey-name">Nama Kunci Akses</Label>
            <Input
                id="passkey-name"
                type="text"
                v-model="name"
                placeholder="Contoh: Laptop kantor, iPhone"
                class="mt-1 block w-full border-foreground/20"
                autofocus
            />
            <p class="text-xs text-muted-foreground">
                Nama membantu Anda mengenali kunci akses ini nantinya.
            </p>
        </div>
